...
const countryListAllIsoData = [
	...
	{ "code": "JP", "code3": "JPN", "name": "Japan (airport only)", "number": "392" },
...
var numCountriesVisited = countryListAllIsoData.length - 1
var numStatesVisited = usStates.length - 1

if (numCountriesVisited > 0) {
	countryListAllIsoData.forEach(country => {
		if (country.name == countryListAllIsoData[numCountriesVisited - 1].name) {
			visitedCountries += country.name + ' '
		} else if (country.name != countryListAllIsoData[numCountriesVisited].name) {
			visitedCountries += country.name + ', '
		} else {
			visitedCountries += 'and ' + country.name + '.'
		}
	})
} else {
	countryListAllIsoData.forEach(country => {
...
if (numStatesVisited > 0) {
	usStates.forEach(state => {
		if (state.name == usStates[numStatesVisited - 1].name) {
			visitedStates += state.name + ' '
		} else if (state.name != usStates[numStatesVisited].name) {
			visitedStates += state.name + ', '
		} else {
			visitedStates += 'and ' + state.name + '.'
		}
	})
} else {
	usStates.forEach(state => {
...
function intToEnglish(num) {
	...
	if (num < 0) {
		return 'minus ' + intToEnglish(Math.abs(num));
	}
...
document.getElementById("countryCount").innerHTML = intToEnglish(countryListAllIsoData.length)
document.getElementById("countriesVisited").innerHTML = visitedCountries
document.getElementById("statesVisited").innerHTML = visitedStates